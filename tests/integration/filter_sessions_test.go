...
// wires it into a real Engine. Some sessions are tracked by agentchat (via
...
// Uses provider config from /root/.agentchat/config.toml so no env-var API
...
// the real agentchat config file. Unlike setupIntegrationEngine, it does
...
func setupE2EEngine(t *testing.T, projectName string) (*core.Engine, *mockPlatform, func()) {
	...
	cfgPath := "/root/.agentchat/config.toml"
