...
const GITHUB_REPO = "Renaissance-Mind/agent-in-chat-feishu";
...
function fetch(url, redirects = 5) {
  return new Promise((resolve, reject) => {
    ...
    mod
      .get(url, { headers: { "User-Agent": "agentchat-npm" } }, (res) => {
...
async function download(urls) {
  for (const url of urls) {
    try {
      console.log(`[agentchat] Downloading from ${url}`);
      const data = await fetch(url);
      console.log(`[agentchat] Downloaded ${(data.length / 1024 / 1024).toFixed(1)} MB`);
      return data;
    } catch (err) {
      console.warn(`[agentchat] Failed: ${err.message}, trying next source...`);
    }
  }
  throw new Error(
    `[agentchat] Could not download binary from any source.\n` +
      `  Tried: ${urls.join(", ")}\n` +
...
async function main() {
  ...
  console.log(`[agentchat] Platform: ${platform}/${arch}`);
  ...
  if (fs.existsSync(binaryPath)) {
    try {
      ...
      if (out.includes(expectedVer)) {
        console.log(`[agentchat] Binary ${VERSION} already installed, skipping.`);
        return;
      }
      // Don't downgrade: if existing binary is newer, keep it
      const match = out.match(/(\d+\.\d+\.\d+[^\s]*)/);
      if (match && isNewerOrEqual(match[1], expectedVer)) {
        console.log(`[agentchat] Binary ${match[1]} is newer than ${VERSION}, skipping.`);
        return;
      }
      console.log(`[agentchat] Existing binary is outdated, upgrading to ${VERSION}...`);
      fs.unlinkSync(binaryPath);
    } catch {
      console.log(`[agentchat] Replacing existing binary with ${VERSION}...`);
      fs.unlinkSync(binaryPath);
    }
  }
  ...
  if (platform === "darwin") {
    try {
      ...
      console.log(`[agentchat] Removed macOS quarantine attribute`);
    } catch {
      // xattr fails if the attribute doesn't exist, which is fine
    }
  }

  console.log(`[agentchat] Installed to ${binaryPath}`);
}

main().catch((err) => {
  console.error(err.message);
  console.error(
    "[agentchat] Installation failed. You can install manually:\n" +
