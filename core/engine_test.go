...
func registerAgentSessionsForKey(e *Engine, sessionKey string, ids ...string) {
	for _, id := range ids {
		s := e.sessions.NewSideSession(sessionKey, id)
...
func TestProcessInteractiveEvents_AppendsReplyFooterWhenEnabled(t *testing.T) {
	...
	agent := &stubReplyFooterAgent{
		...
		workDir: filepath.Join(homeDir, "codes", "agentchat"),
		report: &UsageReport{
			Buckets: []UsageBucket{{
				Name: "Rate limit",
				Windows: []UsageWindow{{
	...
	want := "answer\n\n*gpt-5.4 · xhigh · 100% left · " + compactReplyFooterPath(filepath.Join(homeDir, "codes", "agentchat")) + "*"
	if sent[0] != want {
...
func TestProcessInteractiveEvents_ReplyFooterPrefersSessionRuntimeState(t *testing.T) {
	...
	agentSession.workDir = filepath.Join(homeDir, "codes", "agentchat")
	...
	want := "answer\n\n*gpt-5.4 · xhigh · 31% left · " + compactReplyFooterPath(filepath.Join(homeDir, "codes", "agentchat")) + "*"
	if sent[0] != want {
		t.Fatalf("final reply = %q, want %q", sent[0], want)
	}
}
...
func TestAgentSystemPrompt_MentionsAttachmentSend(t *testing.T) {
	...
	if !strings.Contains(prompt, "agentchat send --image") {
		t.Fatalf("prompt missing image send instructions: %q", prompt)
	}
	if !strings.Contains(prompt, "agentchat send --file") {
...
func TestCmdHelp_UsesLegacyTextOnPlatformWithoutCardSupport(t *testing.T) {
	...
	if strings.Contains(p.sent[0], "agentchat 帮助") {
...
func TestCmdCurrent_UsesLegacyTextOnPlatformWithoutCardSupport(t *testing.T) {
	...
	if strings.Contains(p.sent[0], "agentchat") {
...
func TestCmdDelete_NoArgsOnCardPlatformShowsDeleteModeCard(t *testing.T) {
	...
	registerAgentSessionsForKey(e, msg.SessionKey, "session-1", "session-2")
...
func TestDeleteMode_ConfirmAndSubmitDeletesSelectedSessions(t *testing.T) {
	...
	e := NewEngine("test", agent, []Platform{p}, "", LangEnglish)
	msg := &Message{SessionKey: "feishu:user1", ReplyCtx: "ctx"}
	registerAgentSessionsForKey(e, msg.SessionKey, "session-1", "session-2", "session-3")
...
func TestDeleteMode_PageNavigationPreservesSelection(t *testing.T) {
	...
	e := NewEngine("test", agent, []Platform{p}, "", LangEnglish)
	msg := &Message{SessionKey: "feishu:user1", ReplyCtx: "ctx"}
	registerAgentSessionsForKey(e, msg.SessionKey, "session-1", "session-2", "session-3", "session-4", "session-5", "session-6", "session-7", "session-8")
...
func TestCmdDir_FeishuPersistsChannelSpecificOverrides(t *testing.T) {
	p := &stubPlatformEngine{n: "plain"}
	baseDir := t.TempDir()
	dirA := filepath.Join(baseDir, "group-a")
	dirB := filepath.Join(baseDir, "group-b")
	...
	dataDir := t.TempDir()
	statePath := filepath.Join(dataDir, "projects", "test.state.json")
	store := NewProjectStateStore(statePath)
	...
	e.cmdDir(p, &Message{SessionKey: "feishu:oc_a", ReplyCtx: "ctx-a"}, []string{"group-a"})
	e.cmdDir(p, &Message{SessionKey: "feishu:oc_b", ReplyCtx: "ctx-b"}, []string{"group-b"})

	if agent.workDir != baseDir {
		t.Fatalf("project workDir = %q, want unchanged %q", agent.workDir, baseDir)
	}
	reloaded := NewProjectStateStore(statePath)
	if got := reloaded.WorkDirOverride(); got != "" {
		t.Fatalf("WorkDirOverride() = %q, want empty", got)
	}
	if got := reloaded.ChannelDirOverride("feishu:oc_a"); got != dirA {
		t.Fatalf("ChannelDirOverride(group A) = %q, want %q", got, dirA)
	}
	if got := reloaded.ChannelDirOverride("feishu:oc_b"); got != dirB {
		t.Fatalf("ChannelDirOverride(group B) = %q, want %q", got, dirB)
	}
	if got := e.dirHistory.List("test:feishu:oc_a"); len(got) != 1 || got[0] != dirA {
		t.Fatalf("history group A = %#v, want [%q]", got, dirA)
	}
	if got := e.dirHistory.List("test:feishu:oc_b"); len(got) != 1 || got[0] != dirB {
		t.Fatalf("history group B = %#v, want [%q]", got, dirB)
	}
}

func TestSessionContextForKey_FeishuChannelDirUsesScopedAgent(t *testing.T) {
	agentName := "test-feishu-channel-dir-agent"
	RegisterAgent(agentName, func(opts map[string]any) (Agent, error) {
		agent := &namedStubWorkDirAgent{name: agentName}
		if workDir, ok := opts["work_dir"].(string); ok {
			agent.workDir = workDir
	...
	if err := os.Mkdir(dirA, 0o755); err != nil {
		t.Fatalf("mkdir group-a: %v", err)
	}
	store := NewProjectStateStore(filepath.Join(t.TempDir(), "projects", "test.state.json"))
	store.SetChannelDirOverride("feishu:oc_a", dirA)
	store.Save()

	e := NewEngine("test", &namedStubWorkDirAgent{name: agentName, stubWorkDirAgent: stubWorkDirAgent{workDir: baseDir}}, []Platform{&stubPlatformEngine{n: "plain"}}, "", LangEnglish)
	e.SetProjectStateStore(store)

	agent, sessions := e.sessionContextForKey("feishu:oc_a")
	if sessions != e.sessions {
		t.Fatal("sessionContextForKey should keep the project session manager for channel dir overrides")
	}
	switcher, ok := agent.(WorkDirSwitcher)
	if !ok {
		t.Fatalf("agent type = %T, want WorkDirSwitcher", agent)
	}
	if got := switcher.GetWorkDir(); got != dirA {
		t.Fatalf("scoped agent workDir = %q, want %q", got, dirA)
	}
}
...
func TestRenderListCard_MakesEveryVisibleSessionClickable(t *testing.T) {
	...
	// owned-session filter (simulates agentchat having created each session).
...
func TestSetupMemoryFile_WritesInstructions(t *testing.T) {
	...
	if !strings.Contains(string(content), "agentchat cron add") {
...
func TestSetupMemoryFile_RefreshesLegacyInstructions(t *testing.T) {
	...
	if !strings.Contains(string(content), "agentchat send --image") {
...
func TestCmdCronSetup_WritesAndReplies(t *testing.T) {
	...
	if !strings.Contains(p.sent[0], "attachment send-back") {
		t.Errorf("reply = %q, want unified agentchat setup success message", p.sent[0])
	}
...
func TestBuildSenderPrompt_Enabled(t *testing.T) {
	...
	expected := "[agentchat sender_id=user123 sender_name=\"Alice\" platform=feishu chat_id=channel42]\nhello world"
	if result != expected {
...
func TestBuildSenderPrompt_EmptyUserName(t *testing.T) {
	...
	expected := "[agentchat sender_id=user1 platform=feishu chat_id=ch]\nhello"
	if result != expected {
		t.Fatalf("got %q, want %q", result, expected)
	}
}
...
func TestBuildSenderPrompt_NameWithSpaces(t *testing.T) {
	...
	expected := "[agentchat sender_id=U999 sender_name=\"Jim Tang\" platform=slack chat_id=C012]\nhi"
	if result != expected {
		t.Fatalf("got %q, want %q", result, expected)
	}
}
...
// filter_external_sessions is enabled, only agentchat-tracked sessions
// appear in /list.
...
// by agentchat and 1 external. Returns (engine, platform, userKey, agentSessions).
...
func TestFeishuSessionListsAreScopedByChatID(t *testing.T) {
	agentSessions := []AgentSessionInfo{
		{ID: "group-a-1", Summary: "Group A first", MessageCount: 5, ModifiedAt: time.Now().Add(-3 * time.Hour)},
		{ID: "group-a-2", Summary: "Group A second", MessageCount: 4, ModifiedAt: time.Now().Add(-2 * time.Hour)},
		{ID: "group-b-1", Summary: "Group B only", MessageCount: 3, ModifiedAt: time.Now().Add(-time.Hour)},
		{ID: "external-1", Summary: "External session", MessageCount: 9, ModifiedAt: time.Now()},
	}
	p := &stubPlatformEngine{n: "plain"}
	e := NewEngine("test", &stubListAgent{sessions: agentSessions}, []Platform{p}, "", LangEnglish)

	a1 := e.sessions.GetOrCreateActive("feishu:oc_group_a:ou_1")
	a1.SetAgentSessionID("group-a-1", "codex")
	a2 := e.sessions.NewSession("feishu:oc_group_a:ou_2", "a2")
	a2.SetAgentSessionID("group-a-2", "codex")
	b1 := e.sessions.GetOrCreateActive("feishu:oc_group_b:ou_1")
	b1.SetAgentSessionID("group-b-1", "codex")

	e.cmdList(p, &Message{SessionKey: "feishu:oc_group_a:ou_3", ReplyCtx: "ctx"}, nil)
	if len(p.sent) != 1 {
		t.Fatalf("expected 1 /list reply, got %d", len(p.sent))
	}
	if !strings.Contains(p.sent[0], "Group A first") || !strings.Contains(p.sent[0], "Group A second") {
		t.Fatalf("/list for group A should show group A sessions:\n%s", p.sent[0])
	}
	if strings.Contains(p.sent[0], "Group B only") || strings.Contains(p.sent[0], "External session") {
		t.Fatalf("/list for group A should hide other chats and external sessions:\n%s", p.sent[0])
	}

	p.sent = nil
	e.cmdSwitch(p, &Message{SessionKey: "feishu:oc_group_a:ou_3", ReplyCtx: "ctx"}, []string{"group-b"})
	if len(p.sent) != 1 {
		t.Fatalf("expected 1 /switch reply, got %d", len(p.sent))
	}
	if strings.Contains(p.sent[0], "Group B only") {
		t.Fatalf("/switch in group A should not match group B session:\n%s", p.sent[0])
	}

	p.sent = nil
	e.cmdSwitch(p, &Message{SessionKey: "feishu:oc_group_a:ou_3", ReplyCtx: "ctx"}, []string{"2"})
	if len(p.sent) != 1 {
		t.Fatalf("expected 1 successful /switch reply, got %d", len(p.sent))
	}
	if !strings.Contains(p.sent[0], "Group A second") {
		t.Fatalf("/switch 2 in group A should select second group A session:\n%s", p.sent[0])
	}
}

func TestFeishuRenderListCardIsScopedByChatID(t *testing.T) {
	agentSessions := []AgentSessionInfo{
		{ID: "group-a-1", Summary: "Group A first", MessageCount: 1, ModifiedAt: time.Now().Add(-time.Hour)},
		{ID: "group-b-1", Summary: "Group B only", MessageCount: 1, ModifiedAt: time.Now()},
	}
	e := NewEngine("test", &stubListAgent{sessions: agentSessions}, []Platform{&stubPlatformEngine{n: "feishu"}}, "", LangEnglish)
	e.sessions.GetOrCreateActive("feishu:oc_group_a").SetAgentSessionID("group-a-1", "codex")
	e.sessions.GetOrCreateActive("feishu:oc_group_b").SetAgentSessionID("group-b-1", "codex")

	card, err := e.renderListCard("feishu:oc_group_a", 1)
	if err != nil {
		t.Fatalf("renderListCard returned error: %v", err)
	}
	if got := countCardActionValues(card, "act:/switch "); got != 1 {
		t.Fatalf("switch action count = %d, want 1", got)
	}
	if _, ok := findCardAction(card, "act:/switch 1"); !ok {
		t.Fatal("expected group A switch action")
	}
	if strings.Contains(card.RenderText(), "Group B only") {
		t.Fatalf("group A list card should hide group B session:\n%s", card.RenderText())
	}
}
