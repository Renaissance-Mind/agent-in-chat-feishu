...
type projectStateData struct {
	...
	ChannelDirOverrides   map[string]string `json:"channel_dir_overrides,omitempty"`
}
...
func (ps *ProjectStateStore) ChannelDirOverride(channel string) string {
	ps.mu.RLock()
	defer ps.mu.RUnlock()
	if ps.state.ChannelDirOverrides == nil {
		return ""
	}
	return ps.state.ChannelDirOverrides[channel]
}

func (ps *ProjectStateStore) SetChannelDirOverride(channel, dir string) {
	ps.mu.Lock()
	defer ps.mu.Unlock()
	if ps.state.ChannelDirOverrides == nil {
		ps.state.ChannelDirOverrides = make(map[string]string)
	}
	ps.state.ChannelDirOverrides[channel] = dir
}

func (ps *ProjectStateStore) ClearChannelDirOverride(channel string) {
	ps.mu.Lock()
	defer ps.mu.Unlock()
	if ps.state.ChannelDirOverrides == nil {
		return
	}
	delete(ps.state.ChannelDirOverrides, channel)
	if len(ps.state.ChannelDirOverrides) == 0 {
		ps.state.ChannelDirOverrides = nil
	}
}
