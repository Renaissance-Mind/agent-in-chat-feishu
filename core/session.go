...
// KnownAgentSessionIDs returns the set of agent session IDs tracked by agentchat.
// This is used to filter agent.ListSessions() output to only sessions owned by
// agentchat, excluding sessions created by external CLI usage in the same work_dir.
...
func (sm *SessionManager) KnownAgentSessionIDs() map[string]struct{} {
	...
	for _, s := range sm.sessions {
		collectSessionAgentIDs(ids, s)
	}
	return ids
}

// KnownAgentSessionIDsForSessionKey returns agent session IDs that belong to
// the same platform/channel as sessionKey. This keeps chat-scoped commands such
// as /list and /switch from crossing group boundaries when multiple chats share
// one agent work_dir.
func (sm *SessionManager) KnownAgentSessionIDsForSessionKey(sessionKey string) map[string]struct{} {
	sm.mu.RLock()
	defer sm.mu.RUnlock()
	if sm.legacyData {
		return nil
	...
	for userKey, sessionIDs := range sm.userSessions {
		if extractWorkspaceChannelKey(userKey) != channelKey {
			continue
		}
...
func collectSessionAgentIDs(ids map[string]struct{}, s *Session) {
	if s == nil {
		return
	}
	s.mu.Lock()
