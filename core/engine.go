...
type Engine struct {
	...
	eventIdleTimeoutMu sync.RWMutex
	...
	// When true, /list etc. only show sessions tracked by agentchat,
...
//
//	[agentchat sender_id=ou_abc123 platform=feishu]
//
...
type ConfigReloadResult struct {
	...
	CommandsUpdated    int
	PlatformsUpdated   int
	IdleTimeoutUpdated bool
}
...
func (e *Engine) ReloadPlatformConfig(platformName string, opts map[string]any) (int, error) {
	updated := 0
	for _, p := range e.platforms {
		if p == nil || p.Name() != platformName {
			continue
		}
		reloader, ok := p.(PlatformConfigReloader)
		if !ok {
			continue
		}
		if err := reloader.ReloadPlatformConfig(opts); err != nil {
			return updated, err
		}
		updated++
	}
...
func (e *Engine) SetEventIdleTimeout(d time.Duration) {
	e.eventIdleTimeoutMu.Lock()
	defer e.eventIdleTimeoutMu.Unlock()
	e.eventIdleTimeout = d
}

func (e *Engine) EventIdleTimeout() time.Duration {
	e.eventIdleTimeoutMu.RLock()
	defer e.eventIdleTimeoutMu.RUnlock()
...
func (e *Engine) ExecuteCronJob(job *CronJob) error {
	...
	// with the workspace path (e.g. "/home/user/project:feishu:oc_xxx").
	...
	} else if scopedAgent, scopedDir, ok := e.channelDirAgent(sessionKey); ok {
		agent = scopedAgent
		workspaceDir = scopedDir
	}
...
func (e *Engine) handleMessage(p Platform, msg *Message) {
	...
	if e.multiWorkspace && wsSessions != nil {
		...
		if scopedAgent, scopedDir, ok := e.channelDirAgent(msg.SessionKey); ok {
			agent = scopedAgent
			resolvedWorkspace = scopedDir
		}
	}
...
func (e *Engine) processInteractiveMessage(p Platform, msg *Message, session *Session) {
	...
	e.processInteractiveMessageWith(p, msg, session, agent, e.sessions, msg.SessionKey, workspaceDir, "")
}
...
func (e *Engine) getOrCreateWorkspaceAgent(workspace string) (Agent, *SessionManager, error) {
	...
	// already decorated. See agentchat#496 and the agentchat/core/runas.go
...
func (e *Engine) ensureWorkspacePool() *workspacePool {
	if e.workspacePool != nil {
		return e.workspacePool
	}
	e.workspacePoolMu.Lock()
	defer e.workspacePoolMu.Unlock()
	if e.workspacePool == nil {
		e.workspacePool = newWorkspacePool(15 * time.Minute)
	}
	return e.workspacePool
...
func (e *Engine) dirScopeKeyForSessionKey(sessionKey string) string {
	if e.multiWorkspace || !sessionListScopedByChannel(sessionKey) {
		return ""
	}
	return extractWorkspaceChannelKey(sessionKey)
}

func (e *Engine) dirHistoryKey(sessionKey string) string {
	if key := e.dirScopeKeyForSessionKey(sessionKey); key != "" {
		return e.name + ":" + key
	}
	return e.name
}

func (e *Engine) channelDirOverride(sessionKey string) string {
	if e.projectState == nil {
		return ""
	}
	key := e.dirScopeKeyForSessionKey(sessionKey)
	if key == "" {
		return ""
	}
	override := strings.TrimSpace(e.projectState.ChannelDirOverride(key))
	if override == "" {
		return ""
	}
	...
	e.projectState.ClearChannelDirOverride(key)
	e.projectState.Save()
	return ""
}

func (e *Engine) effectiveWorkDirForSessionKey(agent Agent, sessionKey string) string {
	if dir := e.channelDirOverride(sessionKey); dir != "" {
		return dir
	}
...
func (e *Engine) channelDirAgent(sessionKey string) (Agent, string, bool) {
	dir := e.channelDirOverride(sessionKey)
	if dir == "" {
		return nil, "", false
	}
	agent, _, err := e.getOrCreateWorkspaceAgent(dir)
	if err != nil {
		slog.Warn("channel_dir: failed to create agent, using project agent",
			"session_key", sessionKey, "work_dir", dir, "error", err)
		return nil, "", false
	}
	return agent, dir, true
}
...
func (e *Engine) getOrCreateInteractiveStateWith(sessionKey string, p Platform, replyCtx any, session *Session, sessions *SessionManager, agentOverride Agent, ccSessionKey string) *interactiveState {
	...
	// Inject per-session env vars so the agent subprocess can call `agentchat cron add` etc.
...
func (e *Engine) processInteractiveEvents(state *interactiveState, session *Session, sessions *SessionManager, sessionKey string, msgID string, turnStart time.Time, stopTypingFn func(), sendDone <-chan error, replyCtx any) {
	...
	idleTimeout := e.EventIdleTimeout()
	...
	for {
		...
		case <-idleCh:
			slog.Error("agent session idle timeout: no events for too long, killing session",
				"session_key", sessionKey, "timeout", idleTimeout, "elapsed", time.Since(turnStart))
...
func (e *Engine) handleCommand(p Platform, msg *Message, raw string) bool {
	...
	default:
		...
		// Not a agentchat command — notify user, then fall through to agent
...
// returned. When enabled, only sessions tracked by agentchat are shown.
...
func (e *Engine) applySessionFilterForKey(sessions []AgentSessionInfo, sm *SessionManager, sessionKey string) []AgentSessionInfo {
	if sessionListScopedByChannel(sessionKey) {
		return filterOwnedSessionsStrict(sessions, sm.KnownAgentSessionIDsForSessionKey(sessionKey))
	}
	return e.applySessionFilter(sessions, sm)
}

func sessionListScopedByChannel(sessionKey string) bool {
	switch extractPlatformName(sessionKey) {
	case "feishu", "lark":
		return true
	default:
		return false
...
// filterOwnedSessions removes agent sessions that are not tracked by agentchat's
...
func filterOwnedSessionsStrict(sessions []AgentSessionInfo, known map[string]struct{}) []AgentSessionInfo {
	if known == nil {
		return sessions
	}
	filtered := make([]AgentSessionInfo, 0, len(sessions))
...
func (e *Engine) cmdList(p Platform, msg *Message, args []string) {
	...
	if !supportsCards(p) {
		...
		agentSessions = e.applySessionFilterForKey(agentSessions, sessions, msg.SessionKey)
...
func (e *Engine) dirApply(agent Agent, sessions *SessionManager, interactiveKey, sessionKey string, args []string) (errMsg, successMsg string) {
	...
	currentDir := e.effectiveWorkDirForSessionKey(agent, sessionKey)
	historyKey := e.dirHistoryKey(sessionKey)
	channelDirKey := e.dirScopeKeyForSessionKey(sessionKey)
	...
	if len(args) == 1 {
		...
		case "reset":
			...
			if !e.multiWorkspace && channelDirKey == "" {
			...
			if e.projectState != nil {
				if e.multiWorkspace {
					e.projectState.ClearWorkspaceDirOverride(interactiveKey)
				} else if channelDirKey != "" {
					e.projectState.ClearChannelDirOverride(channelDirKey)
				} else {
			...
			if e.dirHistory != nil {
				e.dirHistory.Add(historyKey, baseDir)
			}
	...
	if idx, err := strconv.Atoi(strings.TrimSpace(arg)); err == nil && idx > 0 {
		if e.dirHistory != nil {
			newDir = e.dirHistory.Get(historyKey, idx)
			if newDir == "" {
		...
		if e.dirHistory != nil {
			newDir = e.dirHistory.Previous(historyKey)
			if newDir == "" {
	...
	if e.dirHistory != nil {
		e.dirHistory.Add(historyKey, newDir)
	}
	if e.projectState != nil {
		if e.multiWorkspace {
			...
			e.projectState.ClearWorkDirOverride()
			e.projectState.SetChannelDirOverride(channelDirKey, newDir)
		} else {
...
func (e *Engine) cmdDir(p Platform, msg *Message, args []string) {
	...
	currentDir := e.effectiveWorkDirForSessionKey(agent, msg.SessionKey)
	historyKey := e.dirHistoryKey(msg.SessionKey)
	...
	if len(args) == 0 {
		...
		if e.dirHistory != nil {
			history := e.dirHistory.List(historyKey)
			if len(history) > 0 {
				sb.WriteString("\n\n")
...
func (e *Engine) executeCardAction(cmd, args, sessionKey string) {
	...
	case "/switch":
		...
		agentSessions = e.applySessionFilterForKey(agentSessions, sessions, sessionKey)
...
func (e *Engine) renderDirCard(sessionKey string, page int) (*Card, error) {
	...
	var history []string
	if e.dirHistory != nil {
		history = e.dirHistory.List(e.dirHistoryKey(sessionKey))
	}
	total := len(history)
	totalPages := 1
	if total > 0 {
...
func (e *Engine) executeShellCommand(p Platform, msg *Message, cmd *CustomCommand, args []string) {
	...
	// Prepend the agentchat binary dir on Windows only (native shell fix);
...
func (e *Engine) cmdConfigReload(p Platform, msg *Message) {
	...
	e.reply(p, msg.ReplyCtx, fmt.Sprintf(e.i18n.T(MsgConfigReloaded),
		result.DisplayUpdated, result.PlatformsUpdated, result.ProvidersUpdated, result.CommandsUpdated))
}
...
const ccConnectInstructionMarker = "<!-- agentchat-instructions -->"
...
func (e *Engine) buildSenderPrompt(content, userID, userName, platform, sessionKey string) string {
	...
	if userName != "" {
		...
		return fmt.Sprintf("[agentchat sender_id=%s sender_name=\"%s\" platform=%s chat_id=%s]\n%s", userID, safeName, platform, chatID, content)
	}
	return fmt.Sprintf("[agentchat sender_id=%s platform=%s chat_id=%s]\n%s", userID, platform, chatID, content)
}
...
func (e *Engine) commandContext(p Platform, msg *Message) (Agent, *SessionManager, string, error) {
	if !e.multiWorkspace {
		if agent, _, ok := e.channelDirAgent(msg.SessionKey); ok {
			return agent, e.sessions, msg.SessionKey, nil
		}
...
func (e *Engine) sessionContextForKey(sessionKey string) (Agent, *SessionManager) {
	if !e.multiWorkspace {
		if agent, _, ok := e.channelDirAgent(sessionKey); ok {
			return agent, e.sessions
		}
