...
// SaveFilesToDisk saves file attachments to workDir/.agentchat/attachments/
...
func SaveFilesToDisk(workDir string, files []FileAttachment) []string {
	...
	attachDir := filepath.Join(workDir, ".agentchat", "attachments")
...
type Message struct {
	...
	MarkContextDelivered func()              // called after platform-enriched/current context is sent to the agent
