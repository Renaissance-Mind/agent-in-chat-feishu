...
// Platform abstracts a messaging platform.
...
// PlatformConfigReloader is an optional interface for platforms that can apply
// selected config.toml option changes without restarting the process.
type PlatformConfigReloader interface {
...
// agentchat capabilities (cron scheduling, etc.).
// The prompt is designed to be appended to the agent's existing system prompt.
func AgentSystemPrompt() string {
	return `You are running inside agentchat, a bridge that connects you to messaging platforms.
Your normal text responses are automatically delivered to the user — just reply normally, do NOT use agentchat send for ordinary text replies.
...
When you generate a local image or file that should be sent to the user, use:
  ...
  agentchat send --image /absolute/path/to/image.png
  agentchat send --file /absolute/path/to/report.pdf
  agentchat send --file /absolute/path/to/report.pdf --image /absolute/path/to/chart.png
...
When the user asks you to do something on a schedule (e.g. "每天早上6点帮我总结GitHub trending"), use the Bash tool to run:
  ...
  agentchat cron add --cron "<min> <hour> <day> <month> <weekday>" --prompt "<task description>" --desc "<short label>"
...
Examples:
  agentchat cron add --cron "0 6 * * *" --prompt "Collect GitHub trending repos and send a summary" --desc "Daily GitHub Trending"
  agentchat cron add --cron "0 9 * * 1" --prompt "Generate a weekly project status report" --desc "Weekly Report"
  agentchat cron add --cron "*/2 * * * *" --exec "ipconfig" --session-mode new-per-run --desc "Every 2 min ipconfig"

You can also list or delete cron jobs:
  agentchat cron list
  agentchat cron del <job-id>

### Bot-to-bot relay
When you need to communicate with another bot (e.g. ask another AI agent a question), use:

  agentchat relay send --to <target_project> "<message>"
