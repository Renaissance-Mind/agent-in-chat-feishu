...
// --preserve-env, but having agentchat's spawn argv be the single
...
// preflight and audit packages and only run at startup / via `agentchat
...
// (one chat user typing rapidly) while still re-verifying often enough
