...
func runSessions(args []string) {
	...
	case "show":
		...
		if id == "" {
			...
			fmt.Fprintln(os.Stderr, "Usage: agentchat sessions show <session-id> [-n N]")
			os.Exit(1)
		}
...
func runSessionsShow(dataDir, id string, limit int) {
	...
	if record == nil {
		...
		fmt.Fprintln(os.Stderr, "Use 'agentchat sessions list' to see available sessions.")
		os.Exit(1)
	}
...
func printSessionsUsage() {
	fmt.Println(`Usage: agentchat sessions [command] [options]
...
Examples:
  agentchat sessions                           Interactive TUI browser
  agentchat sessions list                      List all sessions
  agentchat sessions show "mybot:s1"           Show all messages in session
  agentchat sessions show "#1" -n 20           Show last 20 messages of first session`)
}
