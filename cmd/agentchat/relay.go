...
func printRelayUsage() {
	fmt.Println(`Usage: agentchat relay <command> [options]

Commands:
  send      Send a message to another bot via relay

Run 'agentchat relay <command> --help' for details.`)
}

func printRelaySendUsage() {
	fmt.Println(`Usage: agentchat relay send [options] [<target_project> <message>]
...
Examples:
  agentchat relay send --to claude-bot "What's the weather today?"
  agentchat relay send claude-bot What is the weather today`)
}
