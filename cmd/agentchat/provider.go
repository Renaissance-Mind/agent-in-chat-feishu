...
func printProviderUsage() {
	fmt.Println(`Usage: agentchat provider <command> [options]
...
Examples:
  agentchat provider add --project my-backend --name relay --api-key sk-xxx
  agentchat provider add --project my-backend --name bedrock --env CLAUDE_CODE_USE_BEDROCK=1,AWS_PROFILE=bedrock
  agentchat provider list --project my-backend
  agentchat provider remove --project my-backend --name relay
  agentchat provider import --project my-backend
  agentchat provider presets
  agentchat provider global list
  agentchat provider global add --name minimaxi --api-key sk-xxx --base-url https://api.minimaxi.chat/v1`)
}
...
func runProviderGlobal(args []string) {
	if len(args) == 0 {
		fmt.Fprintln(os.Stderr, `Usage: agentchat provider global <command>
...
func runGlobalProviderList(args []string) {
	...
	if len(providers) == 0 {
		...
		fmt.Println("\nAdd one with: agentchat provider global add --name <name> --api-key <key>")
		return
	}
