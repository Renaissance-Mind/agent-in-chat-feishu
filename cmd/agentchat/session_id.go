...
func printAgentSIDUsage() {
	fmt.Println(`Usage: agentchat agent-sid [options]

Print the agent session ID (e.g. Claude Code, Codex, Gemini CLI) for the
current session. This is the ID used for --resume.

The command reads from the persisted session file; no running agentchat
...
Examples:
  agentchat agent-sid                         Auto-detect from env (inside a session)
  agentchat agent-sid -p mybot -s "feishu:chat:user"`)
}
