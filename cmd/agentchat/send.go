...
func resolveSocketPath(dataDir string) string {
	...
	if home, err := os.UserHomeDir(); err == nil {
		return filepath.Join(home, ".agentchat", "run", "api.sock")
	}
	return filepath.Join(".agentchat", "run", "api.sock")
}

func printSendUsage() {
	fmt.Println(`Usage: agentchat send [options] <message>
       agentchat send [options] -m <message>
       agentchat send [options] --stdin < file
       agentchat send [options] --image <path>
       agentchat send [options] --file <path>
       echo "msg" | agentchat send [options] --stdin

Send a message or attachment to an active agentchat session.
...
Examples:
  agentchat send "Daily summary: ..."
  agentchat send -m "Build completed successfully"
  agentchat send --message "Chart generated" --image /tmp/chart.png
  agentchat send --file /tmp/report.pdf
  agentchat send --stdin <<'EOF'
