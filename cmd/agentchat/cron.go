...
func runCronAdd(args []string) {
	...
	// Fallback to env vars (set by agentchat when spawning agent)
	if project == "" {
...
func runCronInfo(args []string) {
	...
	if id == "" {
		fmt.Fprintln(os.Stderr, "Error: job ID is required")
		fmt.Fprintln(os.Stderr, "Usage: agentchat cron info <id> [field]")
		fmt.Fprintln(os.Stderr, "Use 'agentchat cron list' to see all task IDs.")
		os.Exit(1)
	}
...
func runCronEdit(args []string) {
	...
	if id == "" {
		fmt.Fprintln(os.Stderr, "Error: job ID is required")
		fmt.Fprintln(os.Stderr, "Run 'agentchat cron edit --help' for usage.")
		os.Exit(1)
	}
	if field == "" {
...
func printCronUsage() {
	fmt.Println(`Usage: agentchat cron <command> [options]
...
Run 'agentchat cron <command> --help' for details.`)
}

func printCronAddUsage() {
	fmt.Println(`Usage: agentchat cron add [options] [<min> <hour> <day> <month> <weekday> <prompt>]
...
Examples:
  agentchat cron add --cron "0 6 * * *" --prompt "Collect GitHub trending data" --desc "Daily Trending"
  agentchat cron add --cron "*/30 * * * *" --exec "df -h" --desc "Disk usage check"
  agentchat cron add 0 6 * * * Collect GitHub trending data and send me a summary`)
}

func printCronEditUsage() {
	fmt.Println(`Usage: agentchat cron edit <id> <field> <value> [options]
...
Examples:
  agentchat cron edit abc123 cron_expr "0 9 * * *"
  agentchat cron edit abc123 enabled false
  agentchat cron edit abc123 description "Daily standup reminder"
  agentchat cron edit abc123 timeout_mins 60
  agentchat cron edit abc123 mute true`)
}
