...
var requiredFeishuScopes = []string{
	"im:message",
	"im:message:readonly",
...
var requiredFeishuEvents = []string{
...
func runFeishu(args []string) {
	...
	case "help", "--help", "-h":
		printFeishuUsage()
	default:
		fmt.Fprintf(os.Stderr, "Unknown feishu command: %s\n\n", args[0])
		printFeishuUsage()
		os.Exit(1)
	}
}
...
func runFeishuSetup(args []string, requestedMode string) {
	...
	dataDir := fs.String("data-dir", config.DefaultDataDir(), "directory for cache, sessions, and default config")
	platformType := fs.String("platform-type", "", "force platform type: feishu or lark")
	app := fs.String("app", "", "existing app credentials in app_id:app_secret format")
	...
	case feishuSetupModeBind:
		...
		if err != nil {
			exitWithError(fmt.Errorf("app_id/app_secret validation failed: %w", err))
		}
	...
	case feishuSetupModeNew:
		...
		if err != nil {
			...
			fmt.Fprintln(os.Stderr, "Tip: bind an existing app with `agentchat feishu bind --app cli_xxx:sec_xxx`")
			os.Exit(1)
		}
	...
	default:
		exitWithError(fmt.Errorf("unsupported setup mode %q", effectiveMode))
	}

	baseURL := openFeishuBaseURL
	if finalPlatformType == "lark" {
		baseURL = openLarkBaseURL
	}
	cfgPath := expandHomePath(*configPath)
	cfg, err := loadConfigForSetup(cfgPath)
	if err != nil {
		exitWithError(err)
	}
	if cfg.DataDir == "" || cfg.DataDir == config.DefaultDataDir() {
		cfg.DataDir = expandHomePath(*dataDir)
	}
	cfg.Feishu.AppID = resolvedAppID
	cfg.Feishu.AppSecret = resolvedAppSecret
	cfg.Feishu.BaseURL = baseURL
	cfg.ApplyDefaults()
	if err := saveSetupConfig(cfgPath, cfg); err != nil {
		exitWithError(err)
	}
	...
	fmt.Println()
	fmt.Println("✅ Feishu/Lark app configured for agentchat")
	fmt.Printf("   App ID: %s\n", resolvedAppID)
	fmt.Printf("   Config: %s\n", cfgPath)
	fmt.Printf("   Data:   %s\n", cfg.DataDir)
	if ownerOpenID != "" {
	...
	printFeishuPermissionGuidance(resolvedAppID, finalPlatformType)
}

func runFeishuAuthURL(args []string) {
	fs := flag.NewFlagSet("auth-url", flag.ExitOnError)
	configPath := fs.String("config", config.DefaultConfigPath(), "path to config.toml")
	appID := fs.String("app-id", "", "app_id, e.g. cli_xxx")
	platformType := fs.String("platform-type", "feishu", "platform type: feishu or lark")
	_ = fs.Parse(args)

	id := strings.TrimSpace(*appID)
	if id == "" {
		cfg, err := config.Load(expandHomePath(*configPath))
		if err != nil {
			exitWithError(fmt.Errorf("load config or pass --app-id: %w", err))
		}
		id = cfg.Feishu.AppID
		if strings.Contains(cfg.Feishu.BaseURL, "larksuite.com") {
			*platformType = "lark"
		}
	}
	normalizedType, err := normalizeFeishuPlatformType(*platformType)
	...
	printFeishuPermissionGuidance(id, normalizedType)
}

func printFeishuPermissionGuidance(appID, platformType string) {
	base := openFeishuBaseURL
	if platformType == "lark" {
		base = openLarkBaseURL
	}
	fmt.Println()
	fmt.Println("Open this permissions link to add the scopes agentchat needs:")
	fmt.Println(feishuPermissionURL(base, appID))
	fmt.Println()
	fmt.Println("Required events to verify in the developer console:")
	for _, event := range requiredFeishuEvents {
		fmt.Printf("  - %s\n", event)
	}
	fmt.Println()
	fmt.Println("Feishu may still require publishing the app version or admin approval for sensitive scopes.")
}

func feishuPermissionURL(openBaseURL, appID string) string {
	values := url.Values{}
	values.Set("q", strings.Join(requiredFeishuScopes, ","))
	values.Set("op_from", "openapi")
	values.Set("token_type", "tenant")
	return strings.TrimRight(openBaseURL, "/") + "/app/" + url.PathEscape(appID) + "/auth?" + values.Encode()
}
...
func loadConfigForSetup(path string) (config.Config, error) {
	...
	if _, err := toml.DecodeFile(path, &cfg); err != nil {
		return config.Config{}, fmt.Errorf("decode existing config: %w", err)
	}
	return cfg, nil
}

func saveSetupConfig(path string, cfg config.Config) error {
...
func resolveFeishuSetupInputs(mode, app, appID, appSecret string) (effectiveMode, resolvedAppID, resolvedAppSecret string, err error) {
	...
	if mode == feishuSetupModeBind && (appID == "" || appSecret == "") {
		return "", "", "", fmt.Errorf("bind mode requires credentials: use --app cli_xxx:sec_xxx")
	}
...
func runRegistrationFlow(opts registrationFlowOptions) (*registrationFlowResult, error) {
	...
	fmt.Println("Scan with the Feishu/Lark mobile app to create and authorize the app:")
