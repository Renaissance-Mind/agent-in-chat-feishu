...
const (
	githubRepo   = "Renaissance-Mind/agent-in-chat-feishu"
...
// Fetch the newest stable release in the background.
...
// fetchLatestStableFromGitee is kept for compatibility with the old update flow.
...
func getUpdateHintIfAvailable() string {
	...
	if isNewer(cachedVer, version) {
		return fmt.Sprintf("\n📦 Update available: %s → %s  (run: agentchat update)\n", version, cachedVer)
	}
	return ""
}
...
func runUpdate() {
	...
	fmt.Println("Restart agentchat to use the new version.")
}
...
func binaryAssetName(tag string) string {
	...
	name := fmt.Sprintf("agentchat-%s-%s-%s", tag, goos, goarch)
	if goos == "windows" {
		name += ".exe"
	}
	return name
...
func archiveAssetName(tag string) string {
	goos := runtime.GOOS
	goarch := runtime.GOARCH
	base := fmt.Sprintf("agentchat-%s-%s-%s", tag, goos, goarch)
	if goos == "windows" {
		return base + ".zip"
	}
	return base + ".tar.gz"
...
// extractBinaryFromArchive extracts the agentchat binary from a .tar.gz or .zip archive.
...
func extractFromTarGz(archivePath string) (string, error) {
	...
	for {
		...
		if strings.HasPrefix(hdr.Name, "agentchat") {
			tmp, err := os.CreateTemp("", "agentchat-update-*")
			if err != nil {
				return "", err
			}
...
func extractFromZip(archivePath string) (string, error) {
	...
	for _, f := range r.File {
		if !strings.HasPrefix(f.Name, "agentchat") {
			continue
		}
		rc, err := f.Open()
		if err != nil {
...
func checkUpdate() {
	...
	if isNewer(release.TagName, version) {
		hint := "agentchat update"
		if release.Prerelease {
			hint = "agentchat update --pre"
		}
...
// (node_modules/agentchat/bin/) and updates the package.json version to
...
func syncNpmPackageVersion(execPath, newVer string) {
	...
	if err := os.WriteFile(pkgJSON, out, 0o644); err != nil {
		...
		fmt.Println("   please run: npm update -g agentchat")
	} else {
