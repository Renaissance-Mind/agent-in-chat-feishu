...
type Config struct {
	DataDir        string `toml:"data_dir"` // session store directory, default ~/.agentchat
...
// Prevents account bans on platforms with strict API rate limits.
...
type HeartbeatConfig struct {
	...
	SessionKey   string `toml:"session_key,omitempty"`    // target session key (e.g. "feishu:chat:user"); required
...
type ProjectConfig struct {
	...
	// ResetOnIdleMins automatically rotates to a new agentchat session after
	...
	// runs agentchat itself. Requires passwordless sudo to the target user
	...
	// ReplyFooter: true = append a Codex-style footer; nil/false = disable.
	// The footer contains model/reasoning/usage/workdir metadata when available.
	...
	// agentchat, hiding sessions created by direct CLI usage in the same work_dir.
...
func Load(path string) (*Config, error) {
	...
	if cfg.DataDir == "" {
		if home, err := os.UserHomeDir(); err == nil {
			cfg.DataDir = filepath.Join(home, ".agentchat")
		} else {
			cfg.DataDir = ".agentchat"
		}
	}
